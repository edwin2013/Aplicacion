...
function mostrarMantenimeintoUsuariosEditar(UsuarioId, nombre, apellidos, identificacion, rolId, password, carreraId, inicioPractica, finPractica, correo) {
    var esRolPracticante = rolId == ROLPACIENTE;
    carreraId = esRolPracticante ? carreraId : '-1';
    $("#hdfAccion").val('A');
...
var Usuario = function () {
    ...
    this.esRolPracticante = this.rolId == ROLPACIENTE;
    ...
    this.obtenerDatos = function () {
        var datos = JSON.stringify({
            ...
            RolId: this.rolId,
            Correo: this.correo,
            CarreraId: this.esRolPracticante ? this.carreraId : null,
    ...
    this.validarFormulario = function () {
        ...
        if (this.esRolPracticante) {
            ...
            var rangoValido = comprarFechasInicioFinal(this.fechaInicio, this.fechaFin);
            ...
            mensajeError = rangoValido ? mensajeError : (mensajeError + '<p>La fecha inicio debe ser mayor a la fecha fin.</p>');
        }
...
function mostrarFechasPractica() {
    ...
    var esRolPracticante = rolId == parseInt(ROLPACIENTE);
...
function obtenerUsuarios() {
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ rolId: -1 }),//CON EL ROLID -1 OBTIENE TODOS LOS USUARIOS CON TODOS LOS ROLES.
        success: function (data) {
            ocultarLoading();
            var datos = $.parseJSON(data);
            var lista = datos.ListaUsuarios;
